...
@Component
public class EndMoonStep_200_VillagerTransformsIntoDothraki implements EndMoonStep {

    private CardService cardService;
    private CardFactory cardFactory;
    private StackService stackService;

    public EndMoonStep_200_VillagerTransformsIntoDothraki(CardService cardService, CardFactory cardFactory, StackService stackService) {
    ...
    public void execute(EndMoonSettings settings) {
        ...
        stacks.stream().forEach(stack -> {
            ...
            var magicBook = cards.stream().filter(name ->
                name.getName().equalsIgnoreCase("arakh")).findFirst();

            if (villager.isEmpty() == false && magicBook.isEmpty() == false) {
                cardFactory.makeCard(new CardFactorySettings("Dothraki").withPosition(magicBook.get().getPosition()));
