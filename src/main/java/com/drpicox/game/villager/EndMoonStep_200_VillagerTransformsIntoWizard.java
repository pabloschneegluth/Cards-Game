...
@Component
public class EndMoonStep_200_VillagerTransformsIntoWizard implements EndMoonStep {

    private CardService cardService;
    private CardFactory cardFactory;
    private StackService stackService;

    public EndMoonStep_200_VillagerTransformsIntoWizard(CardService cardService, CardFactory cardFactory, StackService stackService) {
    ...
    public void execute(EndMoonSettings settings) {
        ...
        stacks.stream().forEach(stack -> {
            ...
            var magicBook = cards.stream().filter(name ->
                name.getName().equalsIgnoreCase("magic book")).findFirst();

            if (villager.isEmpty() == false && magicBook.isEmpty() == false) {
                cardFactory.makeCard(new CardFactorySettings("Wizard").withPosition(magicBook.get().getPosition()));
