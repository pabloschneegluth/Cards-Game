...
@Component
public class EndMoonStep_900_BuildWood implements EndMoonStep {
    ...
    public EndMoonStep_900_BuildWood(CardFactory cardFactory,  StackService stackService, CardService cardService, IdeaEndMoonStepExecutor ideaEndMoonStepExecutor) {
    ...
    public void execute(EndMoonSettings settings) {
        ideaEndMoonStepExecutor.execute(settings, IDEA_NAME, this::createWood);
    }

    private void createWood(IdeaEndMoonSettings settings) {
        var cards = settings.getStack().getCards();
        int position = settings.getPosition();
        int totalMaterialsNeeded = 0;

        List<String> stack = Arrays.asList("build idea", "villager", "branch", "branch");
        ...
        cardFactory.makeCards(1, new CardFactorySettings("Wood").withPosition(position));
