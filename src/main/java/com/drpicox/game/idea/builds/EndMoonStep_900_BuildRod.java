...
@Component
public class EndMoonStep_900_BuildRod implements EndMoonStep {
    ...
    public EndMoonStep_900_BuildRod(CardFactory cardFactory, StackService stackService, CardService cardService, IdeaEndMoonStepExecutor ideaEndMoonStepExecutor) {
    ...
    public void execute(EndMoonSettings settings) {
        ideaEndMoonStepExecutor.execute(settings, IDEA_NAME, this::createRod);
    }

    private void createRod(IdeaEndMoonSettings settings){
        var cards = settings.getStack().getCards();
        int position = settings.getPosition();
        int totalMaterialsNeeded = 0;

        List<String> stack = Arrays.asList("build idea", "wizard", "wood", "wood", "wood", "diamond");
        ...
        cardFactory.makeCards(1, new CardFactorySettings("Rod").withPosition(position));
