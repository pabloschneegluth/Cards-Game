...
@Component
public class EndMoonStep_900_BuildShotgun implements EndMoonStep {
    ...
    public EndMoonStep_900_BuildShotgun(CardFactory cardFactory, StackService stackService, CardService cardService, IdeaEndMoonStepExecutor ideaEndMoonStepExecutor) {
    ...
    public void execute(EndMoonSettings settings) {
        ideaEndMoonStepExecutor.execute(settings, IDEA_NAME, this::createShotgun);
    }

    private void createShotgun(IdeaEndMoonSettings settings) {
        var cards = settings.getStack().getCards();
        int position = settings.getPosition();

        List<String> stack = Arrays.asList("build idea", "villager", "wood", "wood", "wood", "iron", "iron");
        ...
        cardFactory.makeCards(1, new CardFactorySettings("Shotgun").withPosition(position));
