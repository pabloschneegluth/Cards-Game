...
@Component
public class EndMoonStep_900_BuildMountainKit implements EndMoonStep {
    ...
    public EndMoonStep_900_BuildMountainKit(CardFactory cardFactory, StackService stackService, CardService cardService, IdeaEndMoonStepExecutor ideaEndMoonStepExecutor) {
    ...
    public void execute(EndMoonSettings settings) {
        ideaEndMoonStepExecutor.execute(settings, IDEA_NAME, this::createMountainKit);
    }

    private void createMountainKit(IdeaEndMoonSettings settings) {
        var cards = settings.getStack().getCards();
        int position = settings.getPosition();
        int totalMaterialsNeeded = 0;

        List<String> stack = Arrays.asList("build idea", "villager", "string", "leather");
        ...
        cardFactory.makeCards(1, new CardFactorySettings("Mountain Kit").withPosition(position));
