...
@Component
public class EndMoonStep_900_BuildHammer implements EndMoonStep {
    ...
    public EndMoonStep_900_BuildHammer(CardFactory cardFactory,  StackService stackService, CardService cardService, IdeaEndMoonStepExecutor ideaEndMoonStepExecutor) {
    ...
    public void execute(EndMoonSettings settings) {
        ideaEndMoonStepExecutor.execute(settings, IDEA_NAME, this::createHammer);
    }

    private void createHammer(IdeaEndMoonSettings settings){
        var cards = settings.getStack().getCards();
        int position = settings.getPosition();
        int totalMaterialsNeeded = 0;

        List<String> stack = Arrays.asList("build idea", "villager", "iron", "iron", "stone", "stone");
